...
public class HabitacionDao {
    ...
    public String modificarHabitacion(Connection con,Datos_Habitacion dhb) {
		PreparedStatement pst = null;
		String sql = "UPDATE Habitaciones SET FechaEntrada = ?, FeChaSalida = ?"
				+ "WHERE cedula =?";
		try {
			pst = con.prepareStatement(sql);
			
			pst.setTimestamp(1,new java.sql.Timestamp(dhb.getFechaEntrada().getTime()));
			pst.setTimestamp(2,new java.sql.Timestamp(dhb.getFechaSalida().getTime()));
			pst.setString(3, dhb.getCedula());
